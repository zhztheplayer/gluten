...
class ParquetFileFormat extends FileFormat with DataSourceRegister with Logging with Serializable {
  ...
  override def prepareWrite(
    ...
    if ("true" == sparkSession.sparkContext.getLocalProperty("isNativeApplicable")) {
      ...
      // We're not really using `ParquetOutputFormat[Row]` for writing data here,
      // because we override it in `ParquetOutputWriter`
      // to support appending and dynamic partitioning.  The reason why
...
object ParquetFileFormat extends Logging {
  ...
  private[parquet] def readParquetFootersInParallel(
    ...
    ThreadUtils
      .parmap(partFiles, "readingParquetFooters", 8) {
        currentFile =>
          try {
  ...
  /**
   * Reads Spark SQL schema from a Parquet footer. If a valid serialized Spark SQL schema string can
   * be found in the file metadata, returns the deserialized [[StructType]], otherwise, returns a
   * [[StructType]] converted from the [[org.apache.parquet.schema.MessageType]] stored in this
   * footer.
   */
