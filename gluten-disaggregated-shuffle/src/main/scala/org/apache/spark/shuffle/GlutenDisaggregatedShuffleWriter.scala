...
import org.apache.gluten.config.{GlutenConfig, SortShuffleWriterType}
import org.apache.gluten.memory.memtarget.{MemoryTarget, Spiller}
import org.apache.gluten.runtime.Runtimes
import org.apache.gluten.vectorized.{DisaggregatedShufflePartitionWriterJniWrapper, GlutenSplitResult, ShuffleWriterJniWrapper}
...
class GlutenDisaggregatedShuffleWriter[K, V](
  ...
  private val partitionWriterJniWrapper =
    DisaggregatedShufflePartitionWriterJniWrapper.create(runtime)
    ...
    while (records.hasNext) {
      ...
      if (cb.numRows == 0 || cb.numCols == 0) {
        ...
        if (nativeShuffleWriter == -1L) {
          val mapOutputWriterWrapper =
            DisaggregatedShuffleMapOutputWriterWrapper(
  ...
  def getPartitionLengths: Array[Long] = partitionWriterJniWrapper.getPartitionLengths

}

case class DisaggregatedShuffleMapOutputWriterWrapper(
    executorComponents: ShuffleExecutorComponents,
    shuffleId: Int,
    mapId: Long,
    numPartitions: Int) {

  private var mapOutputWriter: ShuffleMapOutputWriter = _
  private var singleFileMapOutputWriter: SingleSpillShuffleMapOutputWriter = _

  private var currentPartitionId: Int = -1
  private var partitionWriter: ShufflePartitionWriter = _

  private def initMapOutputWriter(): Unit = {
    if (mapOutputWriter == null) {
      mapOutputWriter = executorComponents.createMapOutputWriter(shuffleId, mapId, numPartitions)
    }
  }

  private def initSingleFileMapOutputWriter(): Unit = {
    if (singleFileMapOutputWriter == null) {
      singleFileMapOutputWriter = executorComponents
        .createSingleFileMapOutputWriter(shuffleId, mapId)
        .get()
    }
  }

  def openStream(partitionId: Int): OutputStream = {
    initMapOutputWriter()
  ...
  def getNumBytesWritten(partitionId: Int): Long = {
    if (partitionId != currentPartitionId || partitionWriter == null) {
      throw new IOException(
        s"Partition $partitionId is not opened, current partition is $currentPartitionId")
    }
    partitionWriter.getNumBytesWritten
  }

  // Checksum is not supported in columnar shuffle.
  def transferMapSpillFile(file: String, partitionLengths: Array[Long]): Unit = {
    initSingleFileMapOutputWriter()
    singleFileMapOutputWriter.transferMapSpillFile(
      new File(file),
      partitionLengths,
      Array.empty[Long])
  }

  def commitAllPartitions(): Array[Long] = {
    initMapOutputWriter()
