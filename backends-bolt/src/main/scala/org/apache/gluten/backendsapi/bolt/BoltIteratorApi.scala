...
import org.apache.gluten.backendsapi.bolt.BoltIteratorApi.unescapePathName
...
import org.apache.spark.shuffle.ShuffleReaderIteratorWrapper
...
/**
 * A wrapper iterator pass to JNI, so that we can access the shuffle reader information if the iter
 * is a shuffle reader iterator
 *
 * @param iter
 *   upstream iterator
 * @param readerWrapper
 *   a ShuffleReaderIteratorWrapper object that hold shuffle reader information and streams
 */
class ShuffleReaderInIterator(
    iter: Iterator[ColumnarBatch],
    val readerWrapper: ShuffleReaderIteratorWrapper)
  extends ColumnarBatchInIterator(BackendsApiManager.getBackendName, iter.asJava) {
  def getReaderWrapper: ShuffleReaderIteratorWrapper = readerWrapper
}

/** Wrap as a WholeStageIteratorWrapper so that the user may know it is a whole stage iterator */
class WholeStageIteratorWrapper[+T](val delegate: Iterator[T], val inner: ColumnarBatchOutIterator)
  extends Iterator[T] {
  override def hasNext: Boolean = delegate.hasNext
  override def next(): T = delegate.next()
  def getInner: ColumnarBatchOutIterator = inner
}

class BoltIteratorApi extends IteratorApi with Logging {
  private def setFileSchemaForLocalFiles(
      localFilesNode: LocalFilesNode,
      fileSchema: StructType,
      fileFormat: ReadFileFormat): LocalFilesNode = {
    if (
      // Note: for orc/dwrf files, let's pass the fileSchema regardless whether UseColumnName is
      // true or false, because in case the orc file was generated by Hive 2.0 or earlier, the
      // column names are like _col0, _col1,..., in which case, Bolt will need to refactor column
      // names using the full table schema.
      ((fileFormat == ReadFileFormat.OrcReadFormat || fileFormat == ReadFileFormat.DwrfReadFormat))
      || (fileFormat == ReadFileFormat.ParquetReadFormat && !BoltConfig.get.parquetUseColumnNames)
    ) {
  ...
  override def genSplitInfo(
    ...
    val preferredLocations =
      filePartitions.flatMap(p => SoftAffinity.getFilePartitionLocations(p))
  ...
  private def getFileInfo(partitionedFiles: Seq[PartitionedFile]): Seq[(JLong, JLong)] = {
    partitionedFiles.map(f => (JLong.valueOf(f.fileSize), JLong.valueOf(f.modificationTime)))
  }
  ...
  /**
   * create ShuffleReaderIteratorWrapper if input iterator is from shuffle reader, otherwise
   * ColumnarBatchInIterator
   */
  private def toColumnarBatchInIterators(inputIterators: Seq[Iterator[ColumnarBatch]]) = {
    inputIterators.map {
      iter =>
        {
          iter match {
            case withMetricsIterator: ShuffleReaderWithMetricsIterator =>
              withMetricsIterator.delegate match {
                case wrapper: ShuffleReaderIteratorWrapper =>
                  new ShuffleReaderInIterator(iter, wrapper)
                case _ =>
                  new ColumnarBatchInIterator(BackendsApiManager.getBackendName, iter.asJava)
              }
            case _ =>
              new ColumnarBatchInIterator(BackendsApiManager.getBackendName, iter.asJava)
          }
  ...
  override def genFirstStageIterator(
    ...
    assert(
      ...
      "Bolt backend only accept GlutenPartition.")
    ...
    val resIter: ColumnarBatchOutIterator =
      transKernel.createKernelWithBatchIterator(
        ...
        if (inputIterators.nonEmpty) toColumnarBatchInIterators(inputIterators) else null,
        partitionIndex,
