...
import org.apache.gluten.extension.{PushdownProjectExecBeforeGeneratorRule, RemoveProjectExecBeforeGeneratorRule}
...
class BoltRuleApi extends RuleApi {
  import BoltRuleApi._
...
object BoltRuleApi {
  ...
  private def injectLegacy(injector: LegacyInjector): Unit = {
    ...
    injector.injectPreTransform(c => PushdownProjectExecBeforeGeneratorRule.apply(c.session))
    injector.injectPreTransform(_ => RemoveProjectExecBeforeGeneratorRule)
    injector.injectPreTransform(_ => CollapseProjectExecTransformer)
    ...
    injector.injectFinal(
      c => PreventBatchTypeMismatchInTableCache(c.caller.isCache(), Set(BoltBatchType)))
    injector.injectFinal(
