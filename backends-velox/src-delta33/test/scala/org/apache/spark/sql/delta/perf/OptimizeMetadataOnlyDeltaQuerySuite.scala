...
class OptimizeMetadataOnlyDeltaQuerySuite
  ...
  Seq(
    ...
    /**
     * Dates are stored as Int in literals. This test make sure Date columns works and NULL are
     * handled correctly
     */
    new SqlTestParams(
    ...
    /**
     * Partitioned columns should be able to return MIN and MAX data even when there are no column
     * stats
     */
    new SqlTestParams(
  ...
  test("optimization not supported - min-max unsupported data types") {
    ...
    columnNames.foreach(
      colName => checkOptimizationIsNotTriggered(s"SELECT MAX($colName) FROM $tableName"))
  }
  ...
  test("optimization not supported - min-max empty table") {
    ...
    checkOptimizationIsNotTriggered(s"SELECT MIN(Column1), MAX(Column1) FROM $tableName")
  }
  ...
  test("optimization not supported - nested columns") {
    ...
    checkOptimizationIsNotTriggered(s"SELECT MAX(Column1.Id) AS XYZ FROM $tableName")
   ...
   * DELTA_OPTIMIZE_METADATA_QUERY_ENABLED enabled and disabled. And the expected Optimized Query
   * Plan with the flag enabled
   */
  private def checkResultsAndOptimizedPlan(query: String, expectedOptimizedPlan: String): Unit = {
  ...
  /**
   * Verify the query plans and results are the same with/without metadata query optimization. This
   * method can be used to verify cases that we shouldn't trigger optimization or cases that we can
   * potentially improve.
   * @param query
   */
  private def checkOptimizationIsNotTriggered(query: String): Unit = {
    ...
    withSQLConf(DeltaSQLConf.DELTA_OPTIMIZE_METADATA_QUERY_ENABLED.key -> "false") {
      ...
      expectedOptimizedPlan = generateQueryDf.queryExecution.optimizedPlan.canonicalized.toString()
      expectedAnswer = generateQueryDf.collect()
    }
