...
class DeltaDDLNameColumnMappingSuite extends DeltaDDLSuite with DeltaColumnMappingEnableNameMode {
    ...
    withTempDir {
      dir =>
        ...
        withTable(tableName) {
          ...
          checkAnswer(sql(s"SELECT * FROM $tableName"), Seq(Row(1L, "a"), Row(null, "b")))
        }
    }
  }
    ...
    withTempDir {
      dir =>
        withTable("delta_test") {
          ...
          checkAnswer(sql("SELECT * FROM delta_test"), Seq(Row(Row(1L, "a"), 1)))
      ...
      withTempDir {
        dir =>
          ...
          withTable(tblName) {
            ...
            checkAnswer(sql(s"SELECT * FROM $tblName"), Seq(Row(Row(1L, "a"), 1)))
    ...
    withTempDir {
      dir =>
        withTable("delta_test") {
          ...
          val df =
            Seq((1, "IT", "Alice"), (2, "CS", "Bob"), (3, "IT", "Carol")).toDF("id", "dept", "name")
