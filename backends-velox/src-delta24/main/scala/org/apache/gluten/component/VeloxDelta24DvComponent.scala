...
import org.apache.gluten.config.{GlutenConfig, VeloxDelta24Config}
import org.apache.gluten.extension.{DeltaPostTransformRules, OffloadDeltaDvScan}
...
class VeloxDelta24DvComponent extends Component {
  override def name(): String = "velox-delta24-dv"
  override def buildInfo(): Component.BuildInfo =
    Component.BuildInfo("VeloxDelta24Dv", "N/A", "N/A", "N/A")
  ...
  override def injectRules(injector: Injector): Unit = {
    ...
    legacy.injectTransform {
      c =>
        val offload = Seq(OffloadDeltaDvScan())
          .map(_.toStrcitRule())
    ...
    val offloads: Seq[RasOffload] = Seq(
      RasOffload.from[FileSourceScanExec](OffloadDeltaDvScan())
    )
    offloads.foreach(
      offload =>
        ras.injectRasRule(
