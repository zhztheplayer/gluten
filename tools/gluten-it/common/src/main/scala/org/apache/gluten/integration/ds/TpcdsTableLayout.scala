...
import org.apache.gluten.integration.table.TableLayout

import org.apache.spark.sql.types.{DateType, DecimalType, LongType, StringType, StructField, StructType}

case class TpcdsTableLayout(partitioned: Boolean) extends TableLayout {
  import TpcdsTableLayout._

  private val tableSchemas = Map[String, StructType](
    "catalog_sales" -> catalogSalesSchema,
    "catalog_returns" -> catalogReturnsSchema,
    "inventory" -> inventorySchema,
    "store_sales" -> storeSalesSchema,
    "store_returns" -> storeReturnsSchema,
    "web_sales" -> webSalesSchema,
    "web_returns" -> webReturnsSchema,
    "call_center" -> callCenterSchema,
    "catalog_page" -> catalogPageSchema,
    "customer" -> customerSchema,
    "customer_address" -> customerAddressSchema,
    "customer_demographics" -> customerDemographicsSchema,
    "date_dim" -> dateDimSchema,
    "household_demographics" -> householdDemographicsSchema,
    "income_band" -> incomeBandSchema,
    "item" -> itemSchema,
    "promotion" -> promotionSchema,
    "reason" -> reasonSchema,
    "ship_mode" -> shipModeSchema,
    "store" -> storeSchema,
    "time_dim" -> timeDimSchema,
    "warehouse" -> warehouseSchema,
    "web_page" -> webPageSchema,
    "web_site" -> webSiteSchema
  )

  private val partitionColumns = Map(
  ...
  override def listTableNames(): Seq[String] = {
    tableSchemas.keys.toSeq
  }
  ...
  override def getPartitionColumns(tableName: String): Seq[String] = {
    if (partitioned) {
      partitionColumns.getOrElse(tableName, Seq.empty)
    } else {
      Seq.empty
    }
  }
