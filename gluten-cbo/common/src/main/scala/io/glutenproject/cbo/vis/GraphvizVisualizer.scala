...
class GraphvizVisualizer[T <: AnyRef](
    cbo: Cbo[T],
  ...
  def format(): String = {
    ...
    val bestPath = best.path(allGroups)
    val winnerNodes = best.winnerNodes().toSet
    val bestNodes = best.bestNodes().toSet
    ...
    allClusters.foreach {
      cluster =>
        ...
        buf.append(s"    label=${'"'}${describeCluster(cluster)}${'"'}\n")
  ...
  private def describeCluster(cluster: CboCluster[T]): String = {
    s"[Cluster ${cluster.id()}]"
  }
...
object GraphvizVisualizer {
  ...
  private object FakeBestFinder {
    private class FakeBest[T <: AnyRef](
      ...
      override def path(allGroups: Int => CboGroup[T]): Best.KnownCostPath[T] = {
        Best.KnownCostPath(
  ...
  def apply[T <: AnyRef](
    ...
    new GraphvizVisualizer(cbo, allClusters, allGroups, fakeBest)
  }
  ...
  def apply[T <: AnyRef](
    ...
    val allClusters = memoState.allClusters()
    val allGroups = memoState.allGroups()
    new GraphvizVisualizer(cbo, allClusters, allGroups, best)
  }
}
