 ...
 * recompute B. If during the procedure C or D or some of their children get invalidate again, then
 ...
 * adjusted, then one of B's sub-tree gets invalidated. Since we apply the adjustment right after
 * the back-dependency (B -> A) is established, algo can still recognize (B -> A)'s removal and
 * recompute B. So this corner case is also handled correctly. The above is a simplified example
  ...
  private class DpZipperAlgoResolver[
    ...
    private def solveYRec(
      ...
      def loop(): Unit = {
        while (true) {
          ...
          val xs = algoDef.browseY(thisY).toSet
          ...
          xs.filterNot(x => xSolutions.contains(algoDef.keyOfX(x))).foreach {
            childX =>
              val xOutputs = solveXRec(childX, xCycleDetector, newYCycleDetector)
              val cm = xOutputs.cycleMemory()
              cyclicXs ++= cm.cyclicXs
              cyclicYs ++= cm.cyclicYs
              val xKey = algoDef.keyOfX(childX)
              sBuilder.addYAsBackDependencyOfX(thisY, childX)
              xSolutions += xKey -> xOutputs.output()
              // Try applying adjustment
              // to see if algo caller likes to add some Xs or to invalidate
              // some of the registered solutions.
              adjustment.exploreChildX(adjustmentPanel, childX)
          }
    ...
    private def solveXRec(
      ...
      def loop(): Unit = {
        while (true) {
          // TODO: Why set is way faster at here than regular iterable / seq / list / vector ?
          val ys = algoDef.browseX(thisX).toSet
          ...
          ys.filterNot(y => ySolutions.contains(algoDef.keyOfY(y))).foreach {
            childY =>
              val yOutputs = solveYRec(childY, newXCycleDetector, yCycleDetector)
              val cm = yOutputs.cycleMemory()
              cyclicXs ++= cm.cyclicXs
              cyclicYs ++= cm.cyclicYs
              val yKey = algoDef.keyOfY(childY)
              sBuilder.addXAsBackDependencyOfY(thisX, childY)
              ySolutions += yKey -> yOutputs.output()
              // Try applying adjustment
              // to see if algo caller likes to add some Ys or to invalidate
              // some of the registered solutions.
              adjustment.exploreChildY(adjustmentPanel, childY)
          }
