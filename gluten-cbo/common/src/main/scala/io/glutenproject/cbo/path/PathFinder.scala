...
object PathFinder {
  def apply[T <: AnyRef](cbo: Cbo[T], allGroups: Seq[CboGroup[T]]): PathFinder[T] = {
    builder(cbo, allGroups).build()
  }

  def builder[T <: AnyRef](cbo: Cbo[T], allGroups: Seq[CboGroup[T]]): Builder[T] = {
    Builder[T](cbo, allGroups)
  }

  class Builder[T <: AnyRef] private (cbo: Cbo[T], allGroups: Seq[CboGroup[T]]) {
    ...
    def build(): PathFinder[T] = {
      ...
      PathEnumerator(cbo, allGroups, wizard)
    }
  }

  private object Builder {
    def apply[T <: AnyRef](cbo: Cbo[T], allGroups: Seq[CboGroup[T]]): Builder[T] = {
      new Builder(cbo, allGroups)
    }
  }
  ...
  private class PathEnumerator[T <: AnyRef] private (
    ...
    override def find(canonical: CanonicalNode[T]): Iterable[CboPath[T]] = {
      val all = wizard.prepareForNode(cbo, allGroups, canonical).visit().onContinue {
    ...
    override def find(base: CboPath[T]): Iterable[CboPath[T]] = {
      ...
      val all = wizard.prepareForNode(cbo, allGroups, can).visit().onContinue {
    ...
    private def enumerateFromGroup(
      ...
      group
        ...
        .flatMap(
          can => {
            wizard.prepareForNode(cbo, allGroups, can).visit().onContinue {
    ...
    private def enumerateFromNode(
        ...
        wizard: OutputWizard[T]): Iterable[CboPath[T]] = {
      val childrenGroups = canonical.getChildrenGroups(allGroups)
    ...
    private def diveFromGroup(
      ...
      wizard.prepareForNode(cbo, allGroups, canonical).visit().onContinue {
  ...
  private object PathEnumerator {
    def apply[T <: AnyRef](
      ...
      new PathEnumerator(cbo, allGroups, wizard)
    }
  }
