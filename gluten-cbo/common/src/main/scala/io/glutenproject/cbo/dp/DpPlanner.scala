...
private class DpPlanner[T <: AnyRef] private (
  ...
  private lazy val best: (Best[T], KnownCostPath[T]) = {
    altPropSets.foreach(propSet => memo.memorize(plan, propSet))
    val groupId = rootGroupId
    val memoTable = memo.table()
    val best = findBest(memoTable, groupId)
    (best, best.path(memoTable.allGroups()))
  }
...
object DpPlanner {
  def apply[T <: AnyRef](
      ...
      reqPropSet: PropertySet[T],
      plan: T): CboPlanner[T] = {
    new DpPlanner(cbo, altPropSets: Seq[PropertySet[T]], reqPropSet, plan)
  }
