...
trait CboSuiteBase {
  ...
  case class Group(id: Int) extends LeafLike {
  ...
  object PlanModelImpl extends PlanModel[TestNode] {
    ...
    override def newGroupLeaf(groupId: Int, propSet: PropertySet[TestNode]): TestNode = Group(
      groupId)
  ...
  implicit class MemoImplicits[T <: AnyRef](val memo: Memo[T]) {
    def memorize(cbo: Cbo[T], node: T): CboGroup[T] = {
      memo.memorize(node, cbo.propertySetFactory().get(node))
    }
  }
