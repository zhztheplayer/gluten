...
class WizardSuite extends AnyFunSuite {
  ...
  test("None") {
    ...
    val finder = PathFinder.builder(cbo, state.allGroups).output(OutputWizards.none()).build()
  ...
  test("Prune by maximum depth") {
    ...
    def findWithMaxDepths(maxDepths: Seq[Int]) = {
      val builder = PathFinder.builder(cbo, state.allGroups)
  ...
  test("Prune by pattern") {
    ...
    def findWithPatterns(patterns: Seq[Pattern[TestNode]]) = {
      val builder = PathFinder.builder(cbo, state.allGroups)
      val finder = patterns
        .foldLeft(builder) {
