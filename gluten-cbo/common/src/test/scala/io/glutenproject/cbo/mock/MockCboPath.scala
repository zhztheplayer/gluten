...
import io.glutenproject.cbo.{CanonicalNode, Cbo, Memo}
...
object MockCboPath {
  ...
  def mock[T <: AnyRef](cbo: Cbo[T], node: T, keys: PathKeySet): CboPath[T] = {
    val memo = Memo(cbo)
    val g = memo.memorize(node, cbo.propertySetFactory().get(node))
    assert(g.nodes().size == 1)
    val state = memo.newState()
    val can = g.nodes().head

    def dfs(n: CanonicalNode[T]): CboPath[T] = {
      if (n.isLeaf()) {
        return CboPath.one(cbo, keys, state.allGroups(), n)
      }
      CboPath(
        cbo,
        n,
        n.getChildrenGroups(state.allGroups()).map(_.group(state.allGroups())).map {
          cg =>
            assert(cg.nodes().size == 1)
            dfs(cg.nodes().head)
        }).get
    }
    dfs(can)
