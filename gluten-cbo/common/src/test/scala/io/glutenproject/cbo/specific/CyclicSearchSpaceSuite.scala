...
class CyclicSearchSpaceSuite extends AnyFunSuite {
  ...
  test("Cyclic - find paths, simple self cycle") {
    ...
    def find(can: CanonicalNode[TestNode], depth: Int): Iterable[CboPath[TestNode]] = {
      PathFinder.builder(cbo, mockState.allGroups).depth(depth).build().find(can)
    }
  ...
  test("Cyclic - find best, more nodes") {
    val cbo =
      Cbo[TestNode](
        CostModelImpl,
        PlanModelImpl,
    ...
    assertBestOf(groupA)(best => assert(best.path(mockState.allGroups).cost == LongCost(10)))
    assertBestOf(groupB)(best => assert(best.path(mockState.allGroups).cost == LongCost(6)))
    assertBestOf(groupC)(best => assert(best.path(mockState.allGroups).cost == LongCost(3)))
    assertBestOf(groupD)(best => assert(best.path(mockState.allGroups).cost == LongCost(1)))
    assertBestOf(groupE)(best => assert(best.path(mockState.allGroups).cost == LongCost(4)))
    assertBestOf(groupF)(best => assert(best.path(mockState.allGroups).cost == LongCost(5)))
    assertBestOf(groupG)(best => assert(best.path(mockState.allGroups).cost == LongCost(1)))
    assertBestOf(groupH)(best => assert(best.path(mockState.allGroups).cost == LongCost(1)))
  }
}
