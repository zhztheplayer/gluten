...
class PathFinderSuite extends AnyFunSuite {
  ...
  test("Base") {
    ...
    def find(can: CanonicalNode[TestNode], depth: Int): Iterable[CboPath[TestNode]] = {
      val finder = PathFinder.builder(cbo, state.allGroups).depth(depth).build()
      finder.find(can)
    }
  ...
  test("Dive - basic") {
    ...
    assert(
      path.dive(state.allGroups, 1).map(_.plan()) == List(
        Binary(n1, Unary(n2, Group(3)), Unary(n3, Group(4)))))
    assert(
      path.dive(state.allGroups, 2).map(_.plan()) == List(
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n5, 1))),
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n6, 1)))))
    assert(
      path.dive(state.allGroups, 3).map(_.plan()) == List(
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n5, 1))),
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n6, 1)))))
    assert(
      path.dive(state.allGroups, CboPath.INF_DEPTH).map(_.plan()) == List(
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n5, 1))),
        Binary(n1, Unary(n2, Leaf(n4, 1)), Unary(n3, Leaf(n6, 1)))))
  }
  ...
  test("Find/Dive - binary with different children heights") {
    ...
    def find(can: CanonicalNode[TestNode], depth: Int): Iterable[CboPath[TestNode]] = {
      PathFinder.builder(cbo, state.allGroups).depth(depth).build().find(can)
    }
    ...
    assert(path.plan() == Binary(n1, Group(1), Group(2)))
    assert(
      path.dive(state.allGroups, 1).map(_.plan()).toSeq == List(
