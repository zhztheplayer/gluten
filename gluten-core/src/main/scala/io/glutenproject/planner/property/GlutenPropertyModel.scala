...
object GlutenProperties {
  val SCHEMA_DEF: PropertyDef[SparkPlan, Schema] = new PropertyDef[SparkPlan, Schema] {
    ...
    override def getChildrenPropertyRequirements(plan: SparkPlan): Seq[Schema] = {
  ...
  val CONVENTION_DEF: PropertyDef[SparkPlan, Convention] = new PropertyDef[SparkPlan, Convention] {
    override def getProperty(plan: SparkPlan): Convention = plan match {
      case _: GroupLeafExec => throw new IllegalStateException()
      case Conventions.ColumnarToRowLike(child) => Conventions.ROW_BASED
      case RowToColumnarExec(child) => Conventions.VANILLA_COLUMNAR
      case Conventions.RowToColumnarLike(child) => Conventions.GLUTEN_COLUMNAR
      case p if PlanUtil.isGlutenColumnarOp(p) => Conventions.GLUTEN_COLUMNAR
      case p if PlanUtil.isVanillaColumnarOp(p) => Conventions.VANILLA_COLUMNAR
      case p if p.supportsRowBased => Conventions.ROW_BASED
      case _ => throw new IllegalStateException()
    }

    override def getChildrenPropertyRequirements(plan: SparkPlan): Seq[Convention] = plan match {
      case ColumnarToRowExec(child) => Seq(Conventions.VANILLA_COLUMNAR)
      case Conventions.ColumnarToRowLike(child) => Seq(Conventions.GLUTEN_COLUMNAR)
      case Conventions.RowToColumnarLike(child) => Seq(Conventions.ROW_BASED)
      case _ =>
