...
class PushAggregateThroughJoinSuite extends PlanTest with SharedSparkSession {
  ...
  test("pre-aggregate store_sales for both joins with having filter") {
    ...
    runCaseWithMaxDepth(
      pushdownCase, maxDepth = Int.MaxValue, expectedPushCount = 2, expectedAggCount = 4)
  }
  ...
  test("pre-aggregate store_sales for sum") {
    ...
    runCaseWithMaxDepth(
      pushdownCase, maxDepth = Int.MaxValue, expectedPushCount = 1, expectedAggCount = 3)
  }
  ...
  test("pre-aggregate three-way joins independently under union all") {
    ...
    runCaseWithMaxDepth(pushdownCase, maxDepth = 1, expectedPushCount = 3, expectedAggCount = 9)
    runCaseWithMaxDepth(pushdownCase, maxDepth = 2, expectedPushCount = 6, expectedAggCount = 12)
    runCaseWithMaxDepth(
      pushdownCase, maxDepth = Int.MaxValue, expectedPushCount = 6, expectedAggCount = 12)
  }
  ...
  test("pre-aggregate store_sales for sum on three-way join with maxDepth=1 / maxDepth=2") {
    ...
    runCaseWithMaxDepth(pushdownCase, maxDepth = 1, expectedPushCount = 1, expectedAggCount = 3)
    runCaseWithMaxDepth(pushdownCase, maxDepth = 2, expectedPushCount = 2, expectedAggCount = 4)
    runCaseWithMaxDepth(
      pushdownCase, maxDepth = Int.MaxValue, expectedPushCount = 2, expectedAggCount = 4)
  }
